...
function highlightError(id, state) {
  ...
  if (state) el.classList.add('error');
...
document.getElementById('form').addEventListener('submit', function(e) {
  ...
  // Score (Beispiel: Punkte für verschiedene Bereiche)
  let score = 0;
  let analysis = [];
  // Scorepunkte & Status-Icons
  ...
  // Score und Bewertungstext
  const scorePct = Math.round(score/77*100);
  let rating = scorePct > 80 ? "Sehr gut" : (scorePct > 65 ? "Gut" : (scorePct > 45 ? "Okay" : "Schwach"));
  let color = scorePct > 80 ? "#00ffb3" : (scorePct > 65 ? "#ffb300" : "#f04747");
  let icon = scorePct > 80 ? "🔥" : (scorePct > 65 ? "👍" : "⚠️");
  // Fazit-Box
  ...
  document.getElementById('fazit').innerHTML =
    `<div class="fazit-box">
      <strong>Fazit und Kaufpreisanalyse</strong><br>
      Um eine Bruttorendite von <b>5%</b> zu erreichen, wäre ein Kaufpreis von <b>${formatCurrency(zielPreis)}</b> rentabel.
      ...
      <br><br>Gesamtwertung: <span class="fazit-score">${icon} ${rating} (${score} Punkte)</span>
    </div>`;
  // Chart.js Kreis
  ...
  // Prozent-Zahl mittig
  setTimeout(()=>{ 
    ctx.font = "2.2rem Rubik";
    ctx.textAlign = "center";
    ctx.fillStyle = color;
    ctx.fillText(scorePct + "%", 100, 115);
  }, 350);
  // Analyse Details
  document.getElementById('analysis').innerHTML =
